...
object Day10 extends AoCApp {
    ...
    private def lookAndSay(s: String, offset: Int = 1, seqLength: Int = 1, said: Queue[String] = Queue()): String = {
        if (offset >= s.length) said.mkString + seqLength + s.last
        else {
            val a = s(offset - 1)
            val b = s(offset)
            if (a == b) lookAndSay(s, offset + 1, seqLength + 1, said)
            else lookAndSay(s, offset + 1, 1, said :+ (seqLength.toString + a))
        }
    }
    ...
    val transformedMore = (0 until iterations._2).foldLeft(transformed)(transformer)
