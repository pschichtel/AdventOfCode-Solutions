...
import tel.schich.adventofcode.shared.Parser.{parseAllSeparated, parseIndentedLine, parseLine, parseLineBreak, parseLines, parseNaturalNumber, parseRepeated, parseSelector, parseString, parseWhile}
import tel.schich.adventofcode.shared.{AoCApp, Parser}
import tel.schich.adventofcode.year2020.Day13
import tel.schich.adventofcode.year2022.Day11.Expression.{Add, Multiply, Number, Old}
...
object Day11 extends AoCApp {
    ...
    private val parseInput = {
        ...
        parseLines[Monkey](parseMonkey)
    }

    private val input = parse(Input2022.Day11, parseInput)

    private def applyExpression(old: Long, expr: Expression, divider: Long): Long = expr match {
    ...
    private def monkeyTurn(items: Queue[Long], monkey: Monkey, monkeyItems: Map[Long, Queue[Long]], divider: Long, remainder: Long): (Map[Long, Queue[Long]], Long) = {
    ...
    private def calculateBusiness(divider: Long, iterations: Int): Long = {
        val monkeyItems = input.map(m => (m.id, Queue.from(m.startItems))).toMap

        val checkRemainder = input.map(_.divisibleBy).product

        val (_, inspectCount) = (1 to iterations).foldLeft((monkeyItems, Map.empty[Long, Long])) {
            case ((iterationItems, inspectCount), _) =>
                round(input, iterationItems, inspectCount, divider, checkRemainder)
        }

        inspectCount.values.toSeq.sorted.reverse.take(2).product
    }

    override def solution: (Any, Any) = (calculateBusiness(3, 20), calculateBusiness(1, 10000))
}
